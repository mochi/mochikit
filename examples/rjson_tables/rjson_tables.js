...
SortableManager.prototype = {
    ...
    "loadFromURL": function (format, url) {
        ...
        if (format == "rjson") {
            var uid = "MochiKit_rjson_" + (new Date()).getTime();
            document.write('<iframe style="border: 0px; width: 1px; height: 1px; position: absolute; bottom: 0px; right: 0px; visibility: visible;" name="' + uid + '" id="' + uid + '"><html><head><script type="text/javascript" src="' + url + '"></script></head><body></body></html>');
            d = succeed(true);
        } else if (format == "json") {
            d = loadJSONDoc(url);
        } else {
