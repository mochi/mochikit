...
MochiKit.Selector.Selector.prototype = {
    ...
    parseExpression: function() {
        ...
        while (match = expr.match(/^([^a-z0-9_-])?([a-z0-9_-]+(?:\([^)]*\))?)(.*)/i)) {
            ...
            switch (modifier) {
                ...
                case ':':       params.pseudoClassNames.push(clause); break;
                case '':
                case undefined: params.tagName = clause.toUpperCase(); break;
                default:        abort(repr(expr));
            }
            expr = rest;
        }

        if (expr.length > 0) abort(repr(expr));        
    },
    ...
    buildMatchExpression: function() {
        ...
        if ((clause = params.pseudoClassNames).length > 0)
            for (var i = 0; i < clause.length; i++) {
                ...
                var pseudoClass = match[1], pseudoClassArgument = match[2]
                switch (pseudoClass) {
                    case 'root':
                    ...
                    case 'nth-last-of-type':
                        ...
                        if (!match) throw "Invalid argument to pseudo element nth-child: " + pseudoClassArgument;
                        var a,b;
                        if (match[0] == 'odd') a = 2, b = 1;
                        else if (match[0] == 'even') a = 2, b = 0;
                        else a = match[2] && parseInt(match), b = parseInt(match[3]);
        ...
        if (clause = params.attributes) {
            map(function(attribute) {
                ...
                switch (attribute.operator) {
                    case '=':       conditions.push(value + ' == ' + repr(attribute.value)); break;
                    case '~=':      conditions.push('findValue(' + splitValueBy(' ') + ', ' + repr(attribute.value) + ') > -1'); break;
                    case '^=':      conditions.push(value + '.substring(0, ' + attribute.value.length + ') == ' + repr(attribute.value)); break;
                    case '$=':      conditions.push(value + '.substring(' + value + '.length - ' + attribute.value.length + ') == ' + repr(attribute.value)); break;
                    case '*=':      conditions.push(value + '.match(' + repr(attribute.value) + ')'); break;
    ...
    nthChild: function(element, a, b, reverse, sametag){
        var siblings = filter(function(node){ return node.nodeType == Node.ELEMENT_NODE; }, element.parentNode.childNodes);
        if (sametag) siblings = filter(function(node){ return node.tagName == element.tagName}, siblings); 
        if (reverse) siblings = MochiKit.Iter.reversed(siblings);
        if (a) {
            var actualIndex = findIdentical(siblings, element);
            return ((actualIndex + 1 - b) / a) % 1 == 0;
        } else {
            return b == findIdentical(siblings, element) + 1;
        }
    },
    ...
    findElements: function(scope, axis) {
        ...
        if (axis == "") {
            ...
            if (!scope) throw "> combinator not allowed without preceeding expression";
            scope = filter(function(node){ return node.nodeType == Node.ELEMENT_NODE; }, scope.childNodes);
        } else if (axis == "+") {
            if (!scope) throw "+ combinator not allowed without preceeding expression";
